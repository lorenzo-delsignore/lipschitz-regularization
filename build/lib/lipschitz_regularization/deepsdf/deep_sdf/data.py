        ...
        for class_name in split[dataset]:
            for instance_name in split[dataset][class_name]:
                instance_filename = os.path.join(
                    "C:\\Users\\loren\\Desktop\\RepoBitbucket\\lipschitz-regularization\\lipschitz-regularization\\deepsdf\\data", dataset, class_name, instance_name, instance_name + ".npz"
                )
...
class SDFSamples(BaseDataset):
    ...
    def get_latent_code(self, meta):
        model = create_model(self.opt)
        model.set_input(meta, label=False)
        latent_code = model(meta)
        print(latent_code)
        return latent_code
    ...
    def __getitem__(self, idx):
        ...
        meta["samples"] = unpack_sdf_samples(filename, self.subsample)
        meta["idx"] = idx
        if self.load_ram:
